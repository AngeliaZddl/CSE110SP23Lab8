describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    var firstProduct = await page.$('product-item');
    const button = await firstProduct.evaluateHandle(el => el.shadowRoot.querySelector('button'));
    await button.click();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const prodItems = await page.$$('product-item');
    for(let i = 1; i < prodItems.length; i++){
      const buttonText = await prodItems[i].evaluate(el => el.shadowRoot.querySelector('button').innerText);
      expect(buttonText).toBe('Remove from Cart');
    }
    const cartCount = await page.$eval('#cart-count', el => el.innerText);
    expect(cartCount).toBe('20');  
